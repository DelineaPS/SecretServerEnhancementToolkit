...
function global:Get-SecretServerSecretTemplate
{
	...
	if (($queryresults | Measure-Object | Select-Object -ExpandProperty Count) -gt 0)
	{
		...
		$templatestack = [System.Collections.ArrayList]::Synchronized((New-Object System.Collections.ArrayList))
		...
		# for each SecretServerSecretTemplate passed
		foreach ($queryobject in $queryresults)
		{
			...
			[void]$PowerShell.AddScript(
			{
				...
				Try
				{
					...
					$templatestack.Add($template) | Out-Null
				}
				Catch
				{
					# if an error occurred during New-Object, create a new SecretServerException and return that with the relevant data
					$e = New-Object SecretServerException -ArgumentList ("Error during New SecretServerSecretTemplate object.")
			...
			[void]$PowerShell.AddParameter('templatestack',$templatestack)
